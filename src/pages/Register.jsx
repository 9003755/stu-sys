...
export default function Register() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center vercel-hero-bg py-12 px-4 sm:px-6 lg:px-8">
      <div className="login-shell max-w-md w-full space-y-8 bg-white p-8 rounded-xl border border-[var(--ui-border)] shadow-sm">
        <div>
          <h2 className="mt-6 text-center text-3xl font-bold text-[var(--ui-title)]">
            注册新账号
          </h2>
          <p className="mt-2 text-center text-sm text-[var(--ui-muted)]">
            已有账号？{' '}
            <Link to="/login" className="font-medium text-[var(--ui-primary)] hover:text-[var(--ui-primary-hover)]">
              立即登录
            </Link>
          </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-semibold rounded-lg text-white bg-[var(--ui-primary)] hover:bg-[var(--ui-primary-hover)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
            >
              {loading ? '注册中...' : '注册'}
            </button>
          </div>
